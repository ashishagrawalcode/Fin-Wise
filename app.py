...
def login():
    if request.method == 'POST':
        ...
        # Handle dict vs tuple for different DBs
...
def profile():
    ...
    if request.method == 'POST':
        if 'username' in request.form:
            query_db('UPDATE users SET username=%s, phone=%s, address=%s WHERE id=%s', (request.form['username'], request.form['phone'], request.form['address'], uid), commit=True)
...
def api_trade():
    ...
    # ID index is 0 in tuple, 'id' in dict
